...
import com.android.payment.databinding.ActivityPay2Binding;
import com.android.payment.models.MemberVO;
import com.android.payment.utils.DatabaseHelper;
import com.android.payment.views.adapters.Pay2Adapter;
...
public class Pay2Activity extends AppCompatActivity {
    ...
    ActivityPay2Binding pay2Binding;
...
//    ListView listview_pay2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pay2Binding = DataBindingUtil.setContentView(this, R.layout.activity_pay2);
...
//        Spinner spinner_pay2_sgroup = findViewById(R.id.spinner_pay2_sgroup);
        String spinner_pay2_result = pay2Binding.spinnerPay2Sgroup.getSelectedItem().toString();


        switch (v.getId()) {

            case R.id.btn_pay2_save:
                String name = pay2Binding.etPay2Name.getText().toString();
...
//                    EditText et_pay2_pay = findViewById(R.id.et_pay2_pay);
                    ...
                    int pay2_pay = 0;
                    try{
                        pay2_pay = Integer.parseInt(pay2Binding.etPay2Pay.getText().toString().trim());
...
//                        getPercent(0);
                    ...
                    pay2Binding.etPay2Name.setText(null);
                    pay2Binding.etPay2Pay.setText(null);
...
//        }
    ...
    public void getPercent(int sgroup){
        ...
        pay2Binding.txtPay2Total.setText("총원 : "+ pay2_total_num + "명");
        if (sgroup == 0){
        ...
        pay2Binding.txtPay2Enrollment.setText("재적 : " + pay2_enrollment_num + "명");
        if (sgroup == 0) {
        ...
        pay2Binding.txtPay2Paid.setText("납부 : "+ pay2_paid_num + "명");
        if (sgroup == 0){
            sql = "SELECT * FROM "+TABLE_NAME+" WHERE pay2 = 0 AND tempexcept = 0";
        }else {
            sql = "SELECT * FROM " + TABLE_NAME + " WHERE pay2 = 0 AND tempexcept = 0 AND sgroup = " + sgroup;
        ...
        pay2Binding.txtPay2Unpaid.setText("미납 : "+ pay2_unpaid_num + "명");
        if (pay2_enrollment_num == 0){
            pay2_percent_num = 0;
        }else{
            pay2_percent_num = Math.round((pay2_paid_num *1000/ pay2_enrollment_num)/10.0);
        }
        pay2Binding.txtPay2Percent.setText("납부율 : " + pay2_percent_num + "%");
    ...
    public void getSearchPay2(int pay2, String spinner_pay2_result){
        ...
        Cursor cursor = db.rawQuery(sql, null);
        if (cursor.getCount() > 0) {
            pay2Binding.lvPay2.setVisibility(View.VISIBLE);
            ...
            pay2Binding.lvPay2.setAdapter(pay2Adapter);

        }else {
            pay2Binding.lvPay2.setVisibility(View.GONE);
